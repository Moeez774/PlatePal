...
export const Recipie = (props) => {
  ...
  useEffect(() => {
    ...
    for (let ingredient in props.recipie) {
      if (ingredient.includes("strIngredient") && props.recipie[ingredient] != "") {
        ingredients.push(props.recipie[ingredient])
      }
    }
  ...
  const video = () => {
    window.open(props.recipie.strYoutube)
  }

  //For saving recipie to favourites 

  const setFavourite = () => {
    localStorage.setItem(props.recipie.idMeal, JSON.stringify(props.recipie))
  }

  return (
    <div className='mainBlock sm:mx-5 md:mx-8 lg:mx-0 rounded-3xl flex flex-row lg:justify-center overflow-hidden w-[100vw] lg:p-5 h-[50vh] sm:h-[50vh] xxxl:h-[96vh] lg:space-x-10'>
      <div className='block1 recipie instruct p-3 smx:p-5 grid place-items-center h-[45vh] sm:h-[50vh] lg:h-[48vh] xxxl:h-[91vh] overflow-auto lg:-ml-5 -translate-x-[100%] lg:-translate-x-0' style={isMobile && userClick? mobileTranform: pcSlides===-1? mainSlide: sideSlide} onTouchStart={startTouch} onTouchEnd={endTouch} >
        <p className='text-xs smx:text-sm font-normal text-center sm:text-lg'>{props.recipie.strInstructions}</p>
      </div>
      ...
      <div className='recipie block1 rounded-3xl py-5 smxx:p-5 sm:py-5 sm:px-2 md:p-5 sm:w-[46vw] lg:w-[30vw] h-[45vh] sm:h-[50vh] lg:h-[48vh] xxxl:h-[91vh] grid place-items-center -translate-x-[100%] lg:-translate-x-0' style={isMobile && userClick? mobileTranform: pcSlides===0? mainSlide: sideSlide} onTouchStart={startTouch} onTouchEnd={endTouch}>
        <div className='flex flex-col space-y-3'>
          <div className='flex justify-center'>
            <img className='rounded-3xl w-[15rem] sm:w-[15rem] md:w-[15rem] lg:w-[15rem]' src={props.recipie.strMealThumb} alt="" />
          </div>
          <div className='overflow-hidden'>
            <h1 className='text-center font-bold text-xl smx:text-2xl md:text-3xl lg:text-xl mx-2'>{props.recipie.strMeal}</h1>
          </div>
          ...
          <div className='flex flex-col lg:flex-row mx-auto justify-center font-semibold space-y-2 lg:space-y-0 lg:space-x-3 smxx:text-lg lg:text-sm lgx:text-lg'>
            <p className='p-2 text-center'>Cuisine: {props.recipie.strArea}</p>
            <p className='p-2 text-center'>Category: {props.recipie.strCategory}</p>
          </div>
          ...
          <div className='flex justify-center font-semibold'>
            <button onClick={setFavourite} className='Btn py-2 px-6 sm:px-8 md:py-3 md:px-10 lg:py-2 lg:font-semibold'>Add to favourites</button>
          </div>
        </div>
      </div>
